    ...
    if (!publisher || publisher->is_suspended()) {
      ...
      Message_Block_Ptr end_historic_samples(
        create_control_message(
          END_HISTORIC_SAMPLES, header, std::move(data),
      ...
      SendControlStatus ret = send_w_control(list, header, std::move(end_historic_samples), remote_id);
...
void DataWriterImpl::replay_durable_data_for(const GUID_t& remote_id)
{
  ...
  if (reader_durable) {
    ...
    if (!publisher || publisher->is_suspended()) {
      ...
      DataSampleHeader header;
      Message_Block_Ptr end_historic_samples(create_control_message(END_HISTORIC_SAMPLES, header, std::move(data),
                                                                    SystemTimePoint::now().to_dds_time()));

      this->controlTracker.message_sent();
      guard.release();
      const SendControlStatus ret = send_w_control(list, header, std::move(end_historic_samples), remote_id);
      if (ret == SEND_CONTROL_ERROR) {
...
DataWriterImpl::send_request_ack()
{
  ...
  element->set_sample(std::move(sample));
  ...
  const DDS::Time_t& source_timestamp)
{
  ...
  const DDS::ReturnCode_t ret = register_instance_i(handle, std::move(data), source_timestamp);
  if (ret != DDS::RETCODE_OK) {
    ACE_ERROR_RETURN((LM_ERROR,
                                      ...
                                      const DDS::Time_t& source_timestamp)
{
  ...
  Message_Block_Ptr sample(create_control_message(UNREGISTER_INSTANCE,
                                                  element->get_header(),
                                                  std::move(unregistered_sample_data),
                                                  source_timestamp));
  element->set_sample(std::move(sample));
                      ...
                      const void* real_data)
{
  ...
  ret = create_sample_data_message(std::move(data),
                                   handle,
                                   element->get_header(),
                                   temp,
                                   source_timestamp,
                                   (filter_out != 0));
  element->set_sample(std::move(temp));
                        ...
                        const DDS::Time_t & source_timestamp)
{
  ...
  Message_Block_Ptr sample(create_control_message(DISPOSE_INSTANCE,
                                                  element->get_header(),
                                                  std::move(registered_sample_data),
                                                  source_timestamp));
  element->set_sample(std::move(sample));
...
DataWriterImpl::end_coherent_changes(const GroupCoherentSamples& group_samples)
{
  ...
  Message_Block_Ptr control(
    create_control_message(
      END_COHERENT_CHANGES, header, std::move(data),
      SystemTimePoint::now().to_dds_time()));
  ...
  guard.release();
  if (this->send_control(header, std::move(control)) == SEND_CONTROL_ERROR) {
    ACE_ERROR((LM_ERROR,
...
DataWriterImpl::send_liveliness(const MonotonicTimePoint& now)
{
  if (this->qos_.liveliness.kind == DDS::MANUAL_BY_TOPIC_LIVELINESS_QOS ||
    ...
    Message_Block_Ptr liveliness_msg(
      create_control_message(
        DATAWRITER_LIVELINESS, header, std::move(empty),
        SystemTimePoint::now().to_dds_time()));

    if (this->send_control(header, std::move(liveliness_msg)) == SEND_CONTROL_ERROR) {
      ACE_ERROR_RETURN((LM_ERROR,
                             ...
                             Message_Block_Ptr msg)
{
  ...
  SendControlStatus status = TransportClient::send_control(header, std::move(msg));
  ...
  const DDS::Time_t& source_timestamp)
{
  ...
  if (handle == DDS::HANDLE_NIL || !get_handle_instance(handle)) {
    ...
    const DDS::ReturnCode_t ret = register_instance_i(handle, std::move(serialized), source_timestamp);
  ...
  GUIDSeq* filter_out)
{
  ...
  return write(std::move(serialized), handle, source_timestamp, filter_out, sample.native_data());
}

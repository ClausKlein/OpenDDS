                                   ...
                                   DCPS::SequenceNumber& sequence)
{
  ...
  if (encap.from_encoding(sedp_encoding, DCPS::MUTABLE) &&
    ...
    send_sample(std::move(payload), size, reader, sequence, reader != GUID_UNKNOWN);
  } else {
    result = DDS::RETCODE_ERROR;
  }
                                              ...
                                              DCPS::SequenceNumber& sequence)
{
  ...
  if (encap.from_encoding(sedp_encoding, DCPS::FINAL) &&
    ...
    send_sample(std::move(payload), size, reader, sequence);
  } else {
    result = DDS::RETCODE_ERROR;
  }
...
Sedp::DiscoveryWriter::write_unregister_dispose(const GUID_t& rid, CORBA::UShort pid)
{
  ...
  if (encap.from_encoding(sedp_encoding, DCPS::MUTABLE) &&
      serializer << encap && serializer << plist) {
    // Send
    write_control_msg(std::move(payload), size, DCPS::DISPOSE_UNREGISTER_INSTANCE);
    return DDS::RETCODE_OK;
  } else {
    // Error
    ACE_ERROR((LM_ERROR,
...
Sedp::Writer::end_historic_samples(const GUID_t& reader)
{
  ...
  if (mb.get()) {
    ...
    write_control_msg(std::move(mb), sizeof(reader), DCPS::END_HISTORIC_SAMPLES,
...
Sedp::Writer::request_ack(const GUID_t& reader)
{
  ...
  if (mb.get()) {
    mb->cont()->wr_ptr(sizeof(reader));
    // 'mb' would contain the DSHeader, but we skip it. mb.cont() has the data
    write_control_msg(std::move(mb), sizeof(reader), DCPS::REQUEST_ACK,
                      DCPS::SequenceNumber::SEQUENCENUMBER_UNKNOWN());
  } else {
    ACE_ERROR((LM_ERROR,
  ...
  CORBA::Long tl_kind)
{
  ...
  if (encap.from_encoding(serializer.encoding(), DCPS::FINAL) &&
    ...
    send_sample(std::move(payload), size, reader, sn);
  } else {
    if (DCPS::DCPS_debug_level) {
      ACE_ERROR((LM_ERROR, "(%P|%t) ERROR: "
